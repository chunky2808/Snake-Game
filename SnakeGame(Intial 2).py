...
pygame.mixer.music.load('sounds/start.wav') #added Sounds
pygame.mixer.music.play(0)#For Playing only one time

#pygame.mixer.music.load('mb_coin.wav') #added Sounds
#pygame.mixer.music.play(-1)#For Playing infinetly
...
def gameOver():#cantuse print to print something as it print on console not on screen
	...
	showScore(123456778)
	pygame.mixer.music.stop() #stopping sound when games end
	pygame.mixer.music.load('sounds/die.wav') #added Sounds
	pygame.mixer.music.play(0)#For Playing infinetly
	...
	for event in pygame.event.get(): #give list of event inside pygame eveny
		if event.type == pygame.QUIT:#quit from game
			...
			if event.key == pygame.K_RIGHT or event.key == ord('d'): #ord checks ascii value
				pygame.mixer.music.load('sounds/jump.wav') #sound when direction change
				pygame.mixer.music.play(0)#For Playing only one time
			...
			if event.key == pygame.K_ESCAPE:#WHEN ESCAPE KEY IS PRESSED
				pygame.mixer.music.load('sounds/exit.wav') #sound when press escape
				pygame.mixer.music.play(0)#For Playing only one time
 ...
 #changing snake direction
  ...
  #changing snake direction
	...
	if snakePos[0] == foodPos[0] and snakePos[1] == foodPos[1]: #matching x and y coordinate of snake and food 	
		pygame.mixer.music.load('sounds/food.wav') #sound when eat food
		pygame.mixer.music.play(0)#For Playing only one time				
		score +=1
